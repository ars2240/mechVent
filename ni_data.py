...
with open('ni_cols.txt', 'r') as f:
    col_names = f.read().split('\n')

col_dic = {0: ['duration', 'src_bytes', 'dst_bytes', 'land', 'wrong_fragment', 'urgent', 'protocol_type', 'service',
               'flag'],
           1: ['hot', 'num_failed_logins', 'logged_in', 'num_compromised', 'root_shell',
...
def ni_loader(batch_size=128, use_gpu=False, shared='service', cur_host=0, swap=0.0, cor_test=False):
...
class FLN(nn.Module):
    def __init__(self, train_feat):
        ...
        """
        self.fl0, self.fl1, self.fl2, self.fl3 = FLC(train_feat[0]), FLC(train_feat[1]), FLC(train_feat[2]),\
                                                 FLC(train_feat[3])
