...
def train_val_test_split(df, seed, train_prop, val_prop, test_prop, oversample_by):
    ...
    # oversample positives - train dataset ONLY
    # https://stackoverflow.com/questions/23887881/how-to-repeat-pandas-data-frame
    train_df = df[df['EncID'].isin(train[0])]
    if oversample_by:
        train_df = pd.concat([train_df]*oversample_by)
    val_df = df[df['EncID'].isin(val[0])]
    test_df = df[df['EncID'].isin(test[0])]
    # a little hacky - to normalize comorbidities static pred
    if 'sum_comorb_ccc' in list(train_df.columns):
        maxc = max(train_df['sum_comorb_ccc'])
        minc = min(train_df['sum_comorb_ccc'])
        for df in [train_df, val_df, test_df]:
             new_col = (df.loc[:,'sum_comorb_ccc'] - minc) / (maxc-minc)
             df.loc[:,'sum_comorb_ccc'] = new_col

    return train_df, val_df, test_df

def make_train_val_test(pos_df, neg_df, seed, train_prop, val_prop, test_prop, time_preds, static_preds, pred_hrs, resample_flag, oversample_by: int):
    ...
    for df in [[pos_df,oversample_by], [neg_df,False]]:
        curr_train, curr_val, curr_test = train_val_test_split(df[0], seed, train_prop, val_prop, test_prop,df[1])
        train_df = train_df.append(curr_train)
        val_df = val_df.append(curr_val)
        test_df = test_df.append(curr_test)
...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, data_dict: dict, pred_hrs: float, stat: bool):
        self.encounters = list(data_dict.keys())
        random.shuffle(self.encounters)
    ...
    def __getitem__(self, idx):
        ...
        if not self.stat:
            ...
            return X_time_tensor, X_stat_tensor, y
