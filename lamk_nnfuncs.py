...
def get_preds_labels(model, dataloader, device, stat):
    '''
...
def stat_train_best_model(model, model_id, full_train_data: DataLoader, opt_epoch: int,
                     optimizer, loss, device=torch.device('cuda')):
    '''
    input:
        full train dataloader obejct, train+val sets
    ...
    '''
    for epoch in range(opt_epoch):
        for batch_n, (X_time, X_stat, y) in enumerate(full_train_data):
            X_time = X_time.float().to(device)
            X_stat = X_stat.float().to(device)
            # cross entropy loss takes an int as the target which corresponds
            # to the index of the target class
