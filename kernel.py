...
class Kernel(Explainer):
    ...
    def get_link(self, dim):
        link_f = self.link.f(self.fx[dim])
        if safe_isinstance(link_f, "torch.Tensor"):
            link_f = link_f.numpy()
        link_fnull = self.link.f(self.fnull[dim])
        if safe_isinstance(link_fnull, "torch.Tensor"):
            link_fnull = link_fnull.numpy()
        return link_f, link_fnull
    ...
    def solve(self, fraction_evaluated, dim):
        ...
        if (self.l1_reg not in ["auto", False, 0]) or (fraction_evaluated < 0.2 and self.l1_reg == "auto"):
            ...
            link_f, link_fnull = self.get_link(dim)
            eyAdj_aug = np.hstack((eyAdj, eyAdj - (link_f - link_fnull)))
        ...
        link_f, link_fnull = self.get_link(dim)
        eyAdj2 = eyAdj - self.maskMatrix[:, nonzero_inds[-1]] * (link_f - link_fnull)
