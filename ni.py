...
tr_loader, val_loader, te_loader = adv_forest_loader(batch_size=128, adv_valid=True, c0=c0, c1=c1, head='NI+Share41_best')
# model = FLR(train_feat=[len(c0), len(c1)], nc=2, classes=7)
model = FLNSH(feats=[c0, c1], nc=2, classes=2)
opt = torch.optim.Adam(model.parameters())
loss = nn.CrossEntropyLoss()

cmab = fcmab(model, loss, opt, nc=2, n=100, c='mabLin', head='NI+Share41_FLNSH_Adv', verbose=True)
