...
epochs = 10
# shared = 'service'
# shared = 'dst_host_rerror_rate'
# shared = 'protocol_type'
shared = 'dst_host_srv_serror_rate'
# shared = 'srv_count'
# shared = 'serror_rate'
head = 'f11_model'
keep_best = True
...
check_folder('./logs')
for cur_host in range(4):

    # """
    old_stdout = sys.stdout  # save old output
    if cur_host == 0:
        log_file = open('./logs/' + head + '.log', 'w')  # open log file
    else:
        log_file = open('./logs/' + head + '.log', 'a')  # open log file
    ...
    for epoch in range(epochs):
        ...
        # scheduler.step()
        torch.save(model.state_dict(), './models/' + head + '_' + str(cur_host) + '.pt')
        loss_avg = np.average(loss_list, weights=size)
        if keep_best and loss_avg < best_loss:
            best_loss = loss_avg
            torch.save(model.state_dict(), './models/' + head + '_' + str(cur_host) + '_best.pt')
        if not keep_best:
            best_loss = loss_avg
        print("%d\t%f" % (epoch, loss_avg))

    if keep_best:
        model.load_state_dict(torch.load('./models/' + head + '_' + str(cur_host) + '_best.pt'))

    print('Host\tTrain\tAcc\tTest\tAcc')
    train_loss, train_acc = loss_acc(train_loader, conf_matrix=conf_matrix, head=head + '_train')
    test_loss, test_acc = loss_acc(test_loader, conf_matrix=conf_matrix, head=head + '_test')
